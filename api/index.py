...
RESCORE_TOKEN = "compass-rescore-d4MnKp7Xq2vRhL9z"
...
def admin_rescore(token: str = ""):
    if token != RESCORE_TOKEN:
        raise HTTPException(403, "forbidden")
    with connect() as conn, conn.cursor() as cur:
        cur.execute("TRUNCATE scores")
        cur.execute(RESCORE_SQL)
        cur.execute("SELECT COUNT(*) AS n, MAX(score) AS mx, AVG(score) AS av FROM scores")
        row = cur.fetchone()
    return {"ok": True, "scored": row["n"], "max_score": row["mx"], "avg_score": float(row["av"] or 0)}
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            ...
            SELECT r.incident_number,
                   COALESCE(NULLIF(TRIM(r.category), ''), r.request_type) AS category,
                   r.group_name, r.department, r.request_type, r.request_status,
                   r.reported_date,
                   COALESCE(r.closed_date, r.resolved_date) AS closed_date,
                   r.address, r.zipcode
            FROM requests_311 r
            LEFT JOIN code_violations v ON v.case_number = r.incident_number
            WHERE r.parcel_norm = %s
              AND v.id IS NULL
            ORDER BY r.reported_date DESC NULLS LAST
            LIMIT 200
            """,
            (norm,),
        )
